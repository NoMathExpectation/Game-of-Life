...
# 细胞
## 更新
execute if score $render_when_inactive golSettings matches 0 as @e[tag=gol_cell,scores={golPause=0,golActiveTime=1..}] at @s run function gol:cell/render
execute unless score $render_when_inactive golSettings matches 0 as @e[tag=gol_cell,scores={golPause=0}] at @s run function gol:cell/render
#define score_holder #cooldown
scoreboard players remove #cooldown golCellStatus 1
execute if score #cooldown golCellStatus matches ..0 as @e[tag=gol_cell,scores={golWarp=..0,golPause=..0,golActiveTime=1..}] at @s run function gol:cell/update
...
execute as @e[tag=gol_cell,scores={golWarp=1..,golActiveTime=1..}] at @s run function gol:cell/update
